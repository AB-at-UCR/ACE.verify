...
| Argument | Type | Required | Default | Description |
|---|---|---|---|---|
| `zip_file` | `str` (positional) | Yes | &mdash; | Path to the DFDC-style zip archive |
| `subfolder` | `str` (positional) | Yes | &mdash; | Subfolder name within the zip for temp extraction prefix |
...
| Argument | Type | Required | Default | Description |
|---|---|---|---|---|
| `--h5` | `str` | Yes | &mdash; | Path to the test HDF5 file |
| `--checkpoint` | `str` | Yes | &mdash; | Path to the model checkpoint |
...
| Frame extraction | 0% &ndash; 25% | Extract frames via `FaceProcessor` |
| Inference | 25% &ndash; 50% | Run model forward pass |
| Grad-CAM | 50% &ndash; 75% | Generate attention heatmap overlay |
| Timeline scoring | 75% &ndash; 100% | Compute temporal scores and evidence flags |
