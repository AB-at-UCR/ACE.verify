...
> :material-help-circle: Solutions to common setup issues, GPU/FFMPEG errors, file format problems, and frequently asked questions about the ACE.verify platform.
...
!!! tip "Solution"
    ...
    MediaPipe is required by `utilities/preprocess.py` for face landmark detection and alignment.
...
!!! tip "Solution"
    Your Python version is outside the supported range (`>=3.10, <3.13`), as defined in `aceverify/pyproject.toml`.
...
!!! warning "Cause"
    The GPU does not have enough VRAM for the current batch size or model.
...
!!! warning "Cause"
    PyTorch was installed without CUDA support, or the CUDA drivers are not properly configured.
...
!!! warning "Cause"
    The node does not have GPU resources available, or the NVIDIA device plugin is not installed.
...
!!! warning "Cause"
    FFmpeg is not installed, not in `PATH`, or the `FFMPEG_BIN` environment variable is not set.

!!! tip "Solutions"
    1. Install ffmpeg (see [Setup and Deployment](Setup-and-Deployment.md) for OS-specific instructions)
...
!!! warning "Cause"
    The ffmpeg command failed, possibly due to a corrupt video file or an unsupported codec.
...
!!! warning "Cause"
    The preprocessing pipeline expects a JSON file within the zip archive that maps video filenames to labels. No `.json` file was found.

!!! tip "Solution"
    Ensure your zip archive contains a JSON metadata file in DFDC format:
...
!!! warning "Cause"
    The MTCNN face detector failed to find a face in any of the 16 extracted frames. This can happen if:
...
!!! tip "Solution"
    The video is automatically skipped with a warning:
...
!!! warning "Cause"
    The uploaded or preprocessed file has an extension that is not in the allowed list.
...
!!! warning "Cause"
    The `sanitize_filename()` function (in `utilities/static_media.py:61`) rejects filenames that are empty, contain only unsafe characters, or attempt path traversal.

!!! tip "Solution"
    Use a valid filename with no path separators, no null bytes, and no leading dots.
...
!!! warning "Cause"
    Streamlit versions ≤ 1.50 force non-allowlisted extensions (including `.mp4`) to `Content-Type: text/plain` with `X-Content-Type-Options: nosniff`, which prevents `<video>` playback.

!!! tip "Solution"
    The application automatically calls `utilities.ensure_static_video_mime()` on startup to patch this behavior. If the issue persists:
...
!!! warning "Cause"
    The `save_upload_bytes()` function (in `utilities/static_media.py:124`) performs a path-safety check that rejects any file path outside the `frontend/static/uploads/` directory.

!!! tip "Solution"
    This is a security feature. The upload path is always resolved under the uploads directory to prevent path traversal attacks.
...
!!! info "Expected behavior"
    This is the expected empty state when no media has been uploaded or selected. The application displays this when `st.session_state.active_file_path` is `None`.
...
!!! info "Expected behavior"
    The application is designed to handle this:
...
**Solution**: If you are experiencing a loop, clear your browser cache and reload the page.
...
!!! warning "Cause"
    The `FaceProcessor.extract_frames()` method (in `utilities/preprocess.py:121`) returned zero frames, meaning `cv2.VideoCapture` failed to decode the video.
...
!!! warning "Cause"
    The Grad-CAM computation may have failed to capture activations or gradients from the model's last convolutional layer.

!!! tip "Solution"
    Check that the model architecture has a `Conv2d` layer that can be hooked. The `generate_gradcam()` function (in `utilities/gradcam.py:7`) searches for the last `Conv2d` layer in the model hierarchy.
...
!!! warning "Cause"
    The NVIDIA Container Toolkit is not installed on the host machine.
...
!!! warning "Cause"
    Port mapping or network configuration issue.
...
!!! warning "Cause"
    The `pyproject.toml` may not be correctly positioned in the build context, or package directories are not found.

!!! tip "Solution"
    The Dockerfile copies `aceverify/pyproject.toml` to the root as `pyproject.toml`:
...
!!! warning "Cause"
    The training job is taking longer than the specified timeout, or the job failed to start.
...
!!! warning "Cause"
    The ephemeral Alpine pod failed to start or mount the PVC.
...
!!! warning "Cause"
    The data file was not staged to the PVC, or the path in the job manifest is incorrect.
...
!!! warning "Cause"
    The checkpoint file format is not a dict with a `model_state_dict` key.
...
!!! warning "Cause"
    The HDF5 dataset contains only one label class (all real or all fake), making balanced sampling impossible.

!!! tip "Solution"
    Ensure your HDF5 dataset contains both real (`label=0`) and fake (`label=1`) samples. The error message includes the found counts:
...
!!! warning "Cause"
    The data loading function selected zero samples, possibly due to an empty HDF5 file or a very small `n` value.

!!! tip "Solution"
    Check that the HDF5 file contains data and the `n` parameter is sufficient:
...
!!! warning "Cause"
    The training data loader returned zero batches for a given epoch.
...
!!! warning "Cause"
    The validation loop did not process any samples.
...
> ACE.verify · Powered by AI · Results are probabilistic and for research use only
